...
const getImages = async (session: AxiosInstance, prompt: string) => {
	...
	const url = `${BING_URL}/images/create?q=${urlEncodedPrompt}&rt=3FORM=GENCRE`; // force use rt=3
	console.log(url);
...
export const generateImagesLinks = async (prompt: string, authCookie: string) => {
	if (!authCookie || !prompt) {
		throw new Error('Missing parameters. Check if prompt or cookie present');
	}
	...
	return await getImages(session, prompt);
};
